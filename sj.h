...
static sj_Value sj__error(char *msg) {
    return (sj_Value){ .type = SJ_ERROR, .start = msg, .end = msg + strlen(msg) };
}
...
sj_Value sj_read(sj_Reader *r) {
    sj_Value res;
top:
    if (r->cur == r->end) { return sj__error("unexpected eof"); }
    res.start = r->cur;
    ...
    case '"':
        ...
        res.start = ++r->cur;
        for (;;) {
            if ( r->cur == r->end) { return sj__error("unclosed string"); }
            if (*r->cur ==    '"') { break; }
            if (*r->cur ==   '\\') { r->cur++; }
    ...
    case '}': case ']':
        res.type = SJ_END;
        if (--r->depth < 0) {
            return sj__error(*r->cur == '}' ? "stray '}'" : "stray ']'");
        }
        r->cur++;
        break;
    ...
    default:
        return sj__error("unknown token");
    }
    res.end = r->cur;
    return res;
}
...
bool sj_iter_object(sj_Reader *r, sj_Value obj, sj_Value *key, sj_Value *val) {
    ...
    *val = sj_read(r);
    if (val->type == SJ_END)   { *val = sj__error("unexpected object end"); }
